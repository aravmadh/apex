...
2. Create the workflow parameter shown in the Workflow Designer screenshot. Set **Direction** to **In** and leave **Required** off.

    | Static ID | Label | Data Type | Required |
    | --- | --- | --- | --- |
    | `V_EMPLOYEE_ID` | Employee ID | VARCHAR2 | No |
...
5. Add a **Parallel Flow** named `Complete Onboarding Setup` after the code activity. The Parallel Flow creates two branch nodes. APEX continues after both branches complete.
...
4. Add button `START_ONBOARDING` with label **Start Onboarding**. Add a **Workflow** process that starts `Onboard New Employee`. Under **Parameters**, select **Employee ID** and map `V_EMPLOYEE_ID` to `PXX_EMPLOYEE_ID`.
